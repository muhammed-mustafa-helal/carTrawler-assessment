...
describe('<CarCard />', () => {
    ...
    test('renders vendor logo with the correct image', async () => {
        ...
        expect(screen.getByAltText(car.vendorName)).toHaveAttribute('src', vendorLogo);
    });

    test('navigates to the car details page with the correct carId when view Deal is clicked', async () => {
        render(
            <MemoryRouter>
                <CarCard car={car} />
            </MemoryRouter>
